...
public class WaveManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (zombieFactory == null)
        {
            ...
            if (zombieFactory == null)
            {
                Debug.LogError("ZombieFactory not found! Add ZombieFactory component to scene.");
                zombieFactory = gameObject.AddComponent<ZombieFactory>();
            }
        }
    ...
    private IEnumerator SpawnWave(Wave wave)
    {
        ...
        else
        {
            ...
            while (zombiesSpawned < wave.zombieCount)
            {
                ...
                // For time survival, keep spawning if time hasn't run out
                if (wave.waveType == WaveType.TimeSurvival && zombiesSpawned >= wave.zombieCount)
                {
                    // Reset counter to keep spawning
                    zombiesSpawned = 0;
                }
            }
        }
        ...
        // For standard wave type, we end when all zombies are killed
        // Time survival ends when timer expires
        // Wave survival ends after all sub-waves complete
    }
    ...
    private void SpawnZombie(Wave wave)
    {
        ...
        if (spawnPoint != null)
        {
            ...
            // Use zombie factory to spawn appropriate zombie
            GameObject zombie = zombieFactory.SpawnZombie(spawnPosition, currentWaveIndex + 1);
